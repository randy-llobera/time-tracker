...
export type HistoryExportItem = HistoryItem & {
  userName: string;
  employerName: string;
};
...
export const listHistoryForExport = async (
  input: HistoryInput,
): Promise<HistoryExportItem[]> => {
  const fromDate = input.fromDate ?? null;
  const toDate = input.toDate ?? null;
  ...
  const rows = (await sql`
    ...
    SELECT
      ...
      coalesce(totals.total_break_seconds, 0) AS "totalBreakSeconds"
    FROM selected_work_days
    JOIN users ON users.id = selected_work_days.user_id
    JOIN employers ON employers.id = selected_work_days.employer_id
    LEFT JOIN totals ON totals.work_day_id = selected_work_days.id
    ORDER BY selected_work_days.work_date DESC, selected_work_days.started_at DESC
  `) as HistoryExportItem[];
